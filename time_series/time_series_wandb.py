...
def train(config=None):
    ...
    with wandb.init(config=config):
        ...
        with open(config.data_dir + '/training_set', 'rb') as f:
            training_set = pickle.load(f)

        x, y, train_x, train_y, test_x, test_y = prepare_data_sets(training_set, config.seq_length, config.train_proportion)

        with open(config.data_dir + '/x', 'wb') as f:
            pickle.dump(x, f)
        with open(config.data_dir + '/y', 'wb') as f:
            pickle.dump(y, f)
        with open(config.data_dir + '/train_x', 'wb') as f:
            pickle.dump(train_x, f)
        with open(config.data_dir + '/train_y', 'wb') as f:
            pickle.dump(train_y, f)
        with open(config.data_dir + '/test_x', 'wb') as f:
            pickle.dump(test_x, f)
        with open(config.data_dir + '/test_y', 'wb') as f:
        ...
        for epoch in range(config.num_epochs):
            ...
            wandb.log({"loss": val_loss.item(), "epoch": epoch})
...
def main(tag):
    ...
    sweep_config = {
        ...
        'parameters' : {
            'seq_length' : {'values' : [4, 6, 8, 10]},
            'train_proportion' : {'values' : [0.84, 0.88, 0.92]},
            'num_layers' : {'value' : 1},
            'num_classes' : {'value' : 1},
            'input_size' : {'value' : 1},
            'hidden_size' : {'values' : [1, 2, 4]},
            'learning_rate' : {'values' : [0.001, 0.01, 0.1]},
            'num_epochs' : {'values' : [1000, 2000, 4000]},
            'data_dir' : {'value' : data_dir}
        }

    }

    sweep_id = wandb.sweep(sweep_config, project="time-series-angular")
